...
bool ImGui::Checkbox(int index, const char* label, ImGuiTreeNodeFlags flags, bool* checked, bool* selected)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;

    window->DC.ItemFlags |= flags;

    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(index);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ...
    if (pressed)
    {
        if (IsMouseHoveringRect(check_bb.Min, check_bb.Max))
        {
            *checked = !(*checked);
            MarkItemEdited(id);
        }
    ...
    if (*selected)
    {
        ...
        RenderFrame(total_bb.Min, total_bb.Max, bg_col, false);
        RenderNavHighlight(total_bb, id, ImGuiNavHighlightFlags_TypeThin);
    }
    else
    {
    ...
    if (g.LogEnabled)
        LogRenderedText(&total_bb.Min, *checked ? "[x]" : "[ ]");
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y), label);

    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.ItemFlags | ImGuiItemStatusFlags_Checkable | (*checked ? ImGuiItemStatusFlags_Checked : 0));
    return pressed;
}
