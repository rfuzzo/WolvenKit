...
{
	for (auto& it : templateCache)
	{
		for (uint32_t i = 1; i <= 3; ++i)
		{
			MeshpassType t = (MeshpassType)i;
			ShaderPass* p = it.second.passShaders[t];
			if (p != nullptr)
			{
				//if (p->layout != VK_NULL_HANDLE)
				//{
					//vkDestroyPipelineLayout(engine->_device, p->layout, nullptr);
					//p->layout = VK_NULL_HANDLE;
				//}
				if (p->pipeline != VK_NULL_HANDLE)
				{
					vkDestroyPipeline(engine->_device, p->pipeline, nullptr);
					p->pipeline = VK_NULL_HANDLE;
				}

				if (p->effect)
				{
					p->effect->cleanup(engine->_device);
				}
			}
		}
	}
...
void vkutil::MaterialSystem::build_default_templates()
{
	...
	{
		ShaderEffect* terrainLit = build_effect(engine, "terrain.vert.spv", "terrain.frag.spv");
		ShaderPass* defaultLitTerrainPass = build_shader(engine->_renderPass, terrainBuilder, terrainLit);

		EffectTemplate defaultTerrain;

		defaultTerrain.passShaders[MeshpassType::Transparency] = nullptr;
		defaultTerrain.passShaders[MeshpassType::DirectionalShadow] = nullptr;
		defaultTerrain.passShaders[MeshpassType::Forward] = defaultLitTerrainPass;
		defaultTerrain.defaultParameters = nullptr;
		defaultTerrain.transparency = assets::TransparencyMode::Opaque;
        templateCache["terrain"] = defaultTerrain;
	}
...
void vkutil::MaterialSystem::fill_builders()
{
	...
	}
	{
		...
		forwardBuilder._multisampling.rasterizationSamples = engine->_msaaSamples;
		forwardBuilder._multisampling.sampleShadingEnable = VK_TRUE;
		forwardBuilder._multisampling.minSampleShading = 0.25f;
#endif
    ...
    {
		terrainBuilder.vertexDescription = Vertex::get_vertex_description();

		terrainBuilder._inputAssembly = vkinit::input_assembly_create_info(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP);

		terrainBuilder._rasterizer = vkinit::rasterization_state_create_info(VK_POLYGON_MODE_FILL);
		terrainBuilder._rasterizer.cullMode = VK_CULL_MODE_NONE;//BACK_BIT;

#ifdef USE_MULTISAMPLING
        terrainBuilder._multisampling = vkinit::multisampling_state_create_info();
		terrainBuilder._multisampling.rasterizationSamples = engine->_msaaSamples;
		terrainBuilder._multisampling.sampleShadingEnable = VK_TRUE;
		terrainBuilder._multisampling.minSampleShading = 0.25f;
#endif

		terrainBuilder._colorBlendAttachment = vkinit::color_blend_attachment_state();

        //default depthtesting
		terrainBuilder._depthStencil = vkinit::depth_stencil_create_info(true, true, VK_COMPARE_OP_GREATER_OR_EQUAL);
    }
}
