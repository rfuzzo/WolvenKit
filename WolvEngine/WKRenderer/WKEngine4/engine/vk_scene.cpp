...
void RenderScene::write_object(GPUObjectData* target, Handle<RenderObject> objectID)
{
	...
	object.origin_rad = glm::vec4(renderable->bounds.origin, renderable->visible? renderable->bounds.radius : -1.0f);
...
void RenderScene::merge_meshes(VulkanEngine* engine)
{
	...
	mergedVertexBuffer = engine->create_buffer<Vertex>(total_vertices * sizeof(Vertex), VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, 
		VMA_MEMORY_USAGE_GPU_ONLY);

	mergedIndexBuffer = engine->create_buffer<uint32_t>(total_indices * sizeof(uint32_t), VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
		VMA_MEMORY_USAGE_GPU_ONLY);
...
void RenderScene::refresh_pass(MeshPass* pass)
{
	...
	}
	{
		...
		for (auto o : pass->unbatchedObjects)
		{
			...
			{
				handle = (uint32_t)pass->objects.size();
