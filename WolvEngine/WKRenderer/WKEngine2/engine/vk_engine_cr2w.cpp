...
namespace WolvenEngine
{
    ...
    void VulkanEngine::load(std::string asset)
    {
        if (hasEnding(asset, "w2w"))
        {
            ...
            std::thread t(&WolvenEngine::VulkanEngine::addWorld, this, asset);
            t.join();
            _renderQueue.enqueue(RenderMessage(RenderMessage::MessageType::MeshesLoaded));
        }
        else if (hasEnding(asset, "w2l"))
        {
            //std::thread (&WolvenEngine::VulkanEngine::addLayer, this, asset).detach();
            std::thread t(&WolvenEngine::VulkanEngine::addLayer, this, asset);
            t.join();
            _renderQueue.enqueue(RenderMessage(RenderMessage::MessageType::MeshesLoaded));
        }
