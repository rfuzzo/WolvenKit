...
namespace WolvenEngine
{
	...
	:_allMeshesLoaded(false)
	,_conditionalVisibilityPrepared(false)
	{
	...
	void VulkanEngine::processRenderQueue()
	{
		...
		if (_renderQueue.try_dequeue(m))
		{
            ...
            case RenderMessage::MessageType::AddTerrain:
                ...
                break;
			case RenderMessage::MessageType::MeshesLoaded:
	...
	void VulkanEngine::init_vulkan()
	{
		...
		vkb::PhysicalDevice physicalDevice = selector
			...
			.add_required_extension(VK_EXT_CONDITIONAL_RENDERING_EXTENSION_NAME)
			.select()
			.value();
        ...
        vkCmdBeginConditionalRenderingEXT = (PFN_vkCmdBeginConditionalRenderingEXT)vkGetDeviceProcAddr(_device, "vkCmdBeginConditionalRenderingEXT");
        ...
        vkCmdEndConditionalRenderingEXT = (PFN_vkCmdEndConditionalRenderingEXT)vkGetDeviceProcAddr(_device, "vkCmdEndConditionalRenderingEXT");
	...
	void VulkanEngine::init_commands()
	{
		...
		for (int i = 0; i < FRAME_OVERLAP; i++) {
			...
			VkCommandBufferAllocateInfo cmdAllocInfo = vkinit::command_buffer_allocate_info(_frames[i]._commandPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY);
	...
	*/
	...
	{
		size_t conditionalBufferItems = 0;
		for (auto rit = _renderBuckets.begin(); rit != _renderBuckets.end(); rit++)
		{
			conditionalBufferItems += rit->second.renderObjects.size();
		}
		...
		_conditionalVisibility.resize(conditionalBufferItems);

		_conditionalBufferSize = conditionalBufferItems * sizeof(int32_t);
		_conditionalBuffer = create_buffer(_conditionalBufferSize, VK_BUFFER_USAGE_CONDITIONAL_RENDERING_BIT_EXT, VMA_MEMORY_USAGE_CPU_TO_GPU);
		_mainDeletionQueue.push_function([&]() {
			vmaDestroyBuffer(_allocator, _conditionalBuffer._buffer, _conditionalBuffer._allocation);
		});

		for (auto i = 0; i < _conditionalVisibility.size(); i++) {
			_conditionalVisibility[i] = 1;
		}

        char* visibilityData;
        vmaMapMemory(_allocator, _conditionalBuffer._allocation, (void**)&visibilityData);
        memcpy(visibilityData, _conditionalVisibility.data(), _conditionalBufferSize);
        vmaUnmapMemory(_allocator, _conditionalBuffer._allocation);

        _allMeshesLoaded = true;
	}

    void VulkanEngine::draw_objects_conditional(VkCommandBuffer cmd)
    {
        void* data;
		...
		if (_conditionalVisibilityPrepared)
		{
			int32_t* pCondition = _conditionalVisibility.data();

			//TODO: have the center + radius info packed into a different structure that can be iterated over.
			...
			for (auto rit = _renderBuckets.begin(); rit != _renderBuckets.end(); rit++)
			{
				for (auto it = rit->second.renderObjects.begin(); it != rit->second.renderObjects.end(); ++it)
				{
					...
					*pCondition++ = _frustum.checkSphere(p, it->mesh->_radius) ? 1 : 0;
				}
			}
		...
		else
		{
            ...
            for (auto rit = _renderBuckets.begin(); rit != _renderBuckets.end(); rit++)
            {
                ...
                for (auto it = rit->second.renderObjects.begin(); it != rit->second.renderObjects.end(); ++it, ++roIndex)
                {
                    ...
                    conditionalRenderingBeginInfo.buffer = _conditionalBuffer._buffer;
                    conditionalRenderingBeginInfo.offset = sizeof(int32_t) * roIndex;
			...
			_conditionalVisibilityPrepared = true;
		}
    }

	// this is just for previewing while the loading is happening
